...
export PDSH_SSH_ARGS_APPEND="-lroot -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PreferredAuthentications=publickey"

# Make short hostname only if its not an IP address
__tm_get_hostname(){
    local HOST="$(echo $* | rev | cut -d ' ' -f 1 | rev)"
    if echo $HOST | grep ^[0-9] -q; then
        echo $HOST
    else
        echo $HOST| cut -d . -f 1
    fi
